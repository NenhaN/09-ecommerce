...
if(isset($_POST['civilite'], $_POST['pseudo'], $_POST['password'], $_POST['confirm_password'], $_POST['email'], $_POST['prenom'], $_POST['nom'], $_POST['adresse'], $_POST['ville'], $_POST['code_postal']))
{
    ...
    // $verifPseudo -> objet PDOStatement                                                               $_POST['pseudo'] / GregFormateur
    ...
    // Si l'indice name 'pdc n'est pas défénit, cela veut dire que l'internaute n'a pas coché la case des politiquess de confidentialités
    if(!isset($_POST['pdc']))
    {
    ...
    if(!isset($error))
    {
        // Création d'une clé de hachaagedu mot de passe 
        $_POST['password'] = password_hash($_POST['password'], PASSWORD_BCRYPT);
        // Requete SQL d'insertion (PREPARE + BINDVALUE + EXECUTE)
        // On définit 1 marqueur pour chaque colonne, valeur insérée dans la BDD
        $inserMembre = $bdd->prepare("INSERT INTO membre (pseudo, password, nom, prenom, email, civilite, ville, code_postal, adresse) VALUES (:pseudo, :password, :nom, :prenom, :email, :civilite, :ville, :code_postal, :adresse)");
        // echo '<pre>'; print_r(inserMembre); echo '</pre>';
        ...
        $inserMembre->bindValue(':pseudo',$_POST['pseudo'], PDO::PARAM_STR);
        $inserMembre->bindValue(':password',$_POST['password'], PDO::PARAM_STR);
        $inserMembre->bindValue(':nom',$_POST['nom'], PDO::PARAM_STR);
        $inserMembre->bindValue(':prenom',$_POST['prenom'], PDO::PARAM_STR);
        $inserMembre->bindValue(':email',$_POST['email'], PDO::PARAM_STR);
        $inserMembre->bindValue(':civilite',$_POST['civilite'], PDO::PARAM_STR);
        $inserMembre->bindValue(':ville',$_POST['ville'], PDO::PARAM_STR);
        $inserMembre->bindValue(':code_postal',$_POST['code_postal'], PDO::PARAM_INT);
        $inserMembre->bindValue(':adresse',$_POST['adresse'], PDO::PARAM_STR);

        // On execute la requete préparée
        $inserMembre->execute(); 

        $_SESSION['valid_inscription'] = "<p class='col-6 bg-success text-white text-center mx-auto p-3'>Félicitations ! Vous êtes maintenant inscrit sur le site. Vous pouvez dès à présent vous connecter.</p>";

        // On redirige l'internaute vers la page connexion.php après l'execution de la requete d'insertion, apres l'inscription de l'internaute
